...
import { KAVACH_DELIVERY_SHORT, stripAddressPhrases } from "./kavachAddress";
...
const FOOD_FILLER = new Set(
    (
        ...
        "home address raipur c504 sunita park labhandih ghar mujhe chahiye dikhao batao kuch hai"
    ).split(/\s+/),
);
...
export function extractFoodQuery(text: string): string {
    const cleaned = stripAddressPhrases(text)
        .toLowerCase()
        .replace(/[^a-z0-9\s']/g, " ")
        .split(/\s+/)
        .filter((w) => w && !FOOD_FILLER.has(w) && !/^\d+$/.test(w) && !/^c\d+$/.test(w));
...
export function isAddressOnlyMessage(text: string): boolean {
    ...
    const rest = t
        ...
        .filter(
            (w) =>
                !FOOD_FILLER.has(w) &&
                !/^(inwant|iwant|want|it|is|be|should|will|shall|there|this|that|deliver|delivered|delivery|send|sent|bhej|bhejo|bhejna|do|karo|kar|pe|par|mera|mere|meri|instamart|zepto|blinkit|apollo|pharmeasy|city|flat|house|pincode|pin|code|location|chhattisgarh|india|492001|c-504|tulip|area|near|same|saved|usual)$/.test(w) &&
                !/^c?\d+$/.test(w),
        );
...
export function restaurantListCopy(opts: GuestRestaurant[], dishQuery?: string): string {
    const lines = opts.map((r, i) => {
        const meta = [r.cuisines, r.rating ? `⭐ ${r.rating}` : "", r.eta ? `🕒 ${r.eta}` : ""].filter(Boolean).join(" · ");
        return `${i + 1}. *${r.name}*${meta ? `\n   ${meta}` : ""}`;
    });
    return [
        `Open now on *Swiggy* near 📍 ${KAVACH_DELIVERY_SHORT}${dishQuery ? ` for "${dishQuery}"` : ""}:`,
        ...lines,
        "",
...
export function noOpenRestaurantsCopy(closed: GuestRestaurant[], dishQuery?: string): string {
    ...
    return [
        `Swiggy doesn't show any restaurant taking orders at 📍 ${KAVACH_DELIVERY_SHORT}${dishQuery ? ` for "${dishQuery}"` : ""} right now 🌙`,
...
export function dishListCopy(restaurant: string, dishes: GuestDish[], dishQuery?: string): string {
